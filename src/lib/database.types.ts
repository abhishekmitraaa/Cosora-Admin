...
/*
 * Generated by `supabase gen types`, with ONE hand-maintained region: the chat
 * moderation objects (Phase 1/2 in textile-spark-net) — `account_suspensions`,
 * `chat_block_reasons`, `conversation_reviews`, `flag_patterns`,
 * `keyword_blocklist`, `conversations.status`, `profiles.account_status`, and the
 * `set_account_status` / `resolve_conversation_review` functions.
 *
 * They are hand-written because this repo has no `gen types` credentials — each
 * one was verified against the live schema of project vxdhhgdfubqedfpwfyrb
 * before being added here, not copied from a spec. Regenerating this file will
 * produce them properly and this note can go.
 */
export type Database = {
  ...
  public: {
    ...
    Functions: {
      ...
      /**
       * Chat moderation. Both are SECURITY DEFINER and gated to support /
       * super_admin inside the function, and both RAISE on refusal — so unlike a
       * table UPDATE, `if (error) throw` is a complete check here.
       */
      resolve_conversation_review: {
        Args: { p_reason_id?: string | null; p_resolution: string; p_review_id: string }
        Returns: undefined
      }
      set_account_status: {
        Args: {
          p_conversation_review_id?: string | null
          p_new_status: string
          p_profile_id: string
